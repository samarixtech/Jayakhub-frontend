...
import TransactionDetailSidebar, { TransactionDetail } from "../components/TransactionDetailSidebar";
...
const PaymentsView = () => {
  ...
  const prevPoints = Array.from({ length: maxPrev }, (_, i) => (currentPoints[i] || 0) * 0.8);
    ...
    <div className="w-full max-w-[1200px] mx-auto pb-12 space-y-6">
      ...
      <div ref={reportRef} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          <StatCard label="Total Revenue" value={`$${data.metrics.totalRevenue}`} trend={`↑ ${data.metrics.totalRevenueGrowth}`} icon={<DollarSign className="w-4 h-4 text-emerald-600" />} iconBg="bg-emerald-50" />
          <StatCard label="Net Profit" value={`$${data.metrics.netProfit}`} trend={`↑ ${data.metrics.netProfitGrowth}`} icon={<TrendingUp className="w-4 h-4 text-blue-600" />} iconBg="bg-blue-50" />
          <StatCard label="Platform Fees" value={`$${data.metrics.platformFees}`} trend={data.metrics.platformFeesLabel} icon={<CreditCard className="w-4 h-4 text-amber-600" />} iconBg="bg-amber-50" />
          <StatCard label="Avg Order Value" value={`$${data.metrics.avgOrderValue}`} trend={`↑ ${data.metrics.avgOrderValueGrowth}`} icon={<ShoppingCart className="w-4 h-4 text-purple-600" />} iconBg="bg-purple-50" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            ...
            <div className="flex items-center justify-center mb-5">
              <DonutChart pcts={donutPcts} colors={donutColors} totalValue={`$${data.metrics.totalRevenue}`} />
            </div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-gray-50 rounded-xl p-4">
                ...
                <span className="text-[20px] font-black text-[#1a1a1a]">${data.metrics.taxCollected}</span>
              </div>
              <div className="bg-gray-50 rounded-xl p-4">
                <span className="text-[11px] text-gray-400 font-medium block mb-1">Platform Commission</span>
                <span className="text-[20px] font-black text-[#1a1a1a]">${data.metrics.platformCommission}</span>
              </div>
              <div className="bg-gray-50 rounded-xl p-4">
                <span className="text-[11px] text-gray-400 font-medium block mb-1">Payment Processing</span>
                <span className="text-[20px] font-black text-[#1a1a1a]">${data.metrics.paymentProcessing}</span>
              </div>
              <div className="bg-gray-50 rounded-xl p-4">
                <span className="text-[11px] text-gray-400 font-medium block mb-1">Delivery Costs</span>
                <span className="text-[20px] font-black text-[#1a1a1a]">${data.metrics.deliveryCosts}</span>
              </div>
            </div>
            <div className="bg-[#e8f5ee] rounded-xl px-5 py-3.5 flex justify-between items-center">
              ...
              <span className="text-[20px] font-black text-[#2d6a4f]">${data.metrics.netProfit}</span>
            </div>
          </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-5">
            <div>
              ...
              <p className="text-[12px] text-gray-400 mt-0.5">{data.transactions.items.length} of {data.transactions.totalCount}</p>
            </div>
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[640px]">
              ...
              {data.transactions.items.map((t) => (
                ...
                >
                  <span className="text-[13px] font-bold text-emerald-600">{t.orderId}</span>
                  ...
                  <span className="text-[13px] font-medium text-gray-600">{new Date(t.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                  <span className="text-[13px] font-medium text-gray-600 capitalize">{t.method}</span>
                  <span className={`text-[13px] font-bold text-right ${parseFloat(t.netAmount) < 0 ? "text-red-500" : "text-[#1a1a1a]"}`}>${Math.abs(parseFloat(t.netAmount)).toFixed(2)}</span>
                  <span className="text-[13px] font-medium text-gray-400 text-right">{parseFloat(t.fee) < 0 ? "-" : ""}${Math.abs(parseFloat(t.fee)).toFixed(2)}</span>
                  <span className={`text-[14px] font-black text-right ${parseFloat(t.total) < 0 ? "text-red-500" : "text-[#1a1a1a]"}`}>{parseFloat(t.total) < 0 ? "-" : ""}${Math.abs(parseFloat(t.total)).toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-1 mt-5">
            ...
            <button className="w-8 h-8 flex items-center justify-center rounded-lg bg-[#346853] text-white text-[12px] font-bold">{data.transactions.page}</button>
