...
import { Upload, CreditCard, Car, BookOpen, Info, CheckCircle, FileText } from "lucide-react";
...
export default function StepKycView() {
  ...
  const { execute, isPending } = useServerAction(completeOnboardingAction, {
    onSuccess: () => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                >
                  {isSelected ? <CheckCircle className="h-6 w-6 text-emerald-600" /> : <doc.icon className={`h-6 w-6 ${doc.iconColor}`} />}
                </div>
                <div>
