...
export default function PaymentModal({ open, onOpenChange }: PaymentModalProps) {
    ...
    const displaySubtotal = subtotal > 0 ? subtotal : 16.50;
        ...
        return (
            ...
            >
                <div className="flex flex-col items-center w-full pb-6">
                    <h2 className="text-[22px] font-black text-[#357252] tracking-tight mb-1">JayakHub</h2>
                    <p className="text-[13px] text-[#8ea89a] font-medium mb-6">Restaurant POS</p>
                    ...
                    <div className="w-full space-y-3 mb-6">
                        {cartItems.length > 0 ? (
                            cartItems.map((item, idx) => (
                                <div key={idx} className="flex justify-between text-[14px] text-[#1b2d22] font-medium">
                    ...
                    <div className="text-center mb-6">
                        <p className="text-[14px] text-[#3e5648] font-medium mb-2">Paid via <span className="font-black text-[#111] capitalize">{method}</span></p>
                        <p className="text-[12px] text-[#8ea89a] font-medium">Thank you for dining with us!</p>
                    </div>
                    ...
                    <div className="flex w-full gap-3">
                        ...
                        <button onClick={handleNewOrder} className="flex-1 bg-[#f4f6f8] hover:bg-[#e9ecef] text-[#111] font-bold py-3 rounded-lg flex items-center justify-center transition-colors">
                            New Order
                        </button>
                    </div>
                </div>
    ...
    return (
        ...
        >
            <DialogHeader className="px-5 py-4 border-b border-gray-100 flex flex-row items-center justify-between text-left">
                <DialogTitle className="text-[18px] font-black tracking-tight text-[#111] border-none">Complete Payment</DialogTitle>
            </DialogHeader>

            <div className="p-5">
                {/* Header Total */}
                <div className="bg-[#f2fbf5] rounded-xl flex flex-col items-center justify-center py-4 mb-4">
                    <span className="text-[32px] font-black text-[#357252] leading-none mb-1">${displayTotal.toFixed(2)}</span>
                    <span className="text-[12px] text-[#789684] font-semibold">Total Payable</span>
                </div>
                ...
                <div className="border-t border-dashed border-gray-200 pt-3 flex justify-between items-center mb-4">
                    ...
                    <span className="text-[15px] font-black text-[#111]">${displayTotal.toFixed(2)}</span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                    ...
                    >
                        {method === 'cash' && <Check className="absolute top-1.5 right-1.5 w-3.5 h-3.5 text-white stroke-[3px]" />}
                        <Banknote className={`w-5 h-5 mb-1.5 ${method === 'cash' ? 'text-white' : 'text-[#2a3c30]'} stroke-[2px]`} />
                        <span className={`text-[12px] font-bold ${method === 'cash' ? 'text-white' : 'text-[#111]'}`}>Cash</span>
                    </button>
                    ...
                    >
                        {method === 'card' && <Check className="absolute top-1.5 right-1.5 w-3.5 h-3.5 text-white stroke-[3px]" />}
                        <CreditCard className={`w-5 h-5 mb-1.5 ${method === 'card' ? 'text-white' : 'text-[#2a3c30]'} stroke-[2px]`} />
                        <span className={`text-[12px] font-bold ${method === 'card' ? 'text-white' : 'text-[#111]'}`}>Card</span>
                    </button>
                    ...
                    >
                        {method === 'online' && <Check className="absolute top-1.5 right-1.5 w-3.5 h-3.5 text-white stroke-[3px]" />}
                        <Smartphone className={`w-5 h-5 mb-1.5 ${method === 'online' ? 'text-white' : 'text-[#2a3c30]'} stroke-[2px]`} />
                        <span className={`text-[12px] font-bold ${method === 'online' ? 'text-white' : 'text-[#111]'}`}>Online</span>
                    </button>
                </div>
                ...
                {method === "cash" && (
                    <div className="bg-[#fcfdfd] border border-gray-100 p-3 rounded-xl mb-4 flex flex-col items-center">
                        <span className="text-[#657a8a] text-[11px] font-bold mb-1.5 w-full text-left">Amount Tendered</span>
                        <input
                            type="text"
                        ...
                        {(() => {
                            ...
                            return (
                                <span className={`text-[14px] font-black ${change >= 0 ? 'text-[#1eb589]' : 'text-red-500'}`}>
                ...
                <button
                    ...
                    disabled={!method}
                    className={`w-full font-bold py-3 rounded-xl text-[14.5px] transition-colors ${!method
                        ? 'bg-[#8debb4] text-white cursor-not-allowed opacity-80'
                        : 'bg-[#1eb589] hover:bg-[#159a72] text-white shadow-md'
                        }`}
                >
                    {!method ? "Select Payment Method" : `Confirm ${method.charAt(0).toUpperCase() + method.slice(1)} Payment`}
                </button>
