"use client";
import { LayoutGrid, Sandwich, Globe } from "lucide-react";
    ...
    <aside className="w-[60px] shrink-0 bg-white lg:border-r border-gray-200 flex flex-col items-center py-2 h-full z-10 overflow-y-auto">
      <div className="flex-1 w-full flex flex-col items-center gap-4 pt-1">
        {isPosLoading
          ...
          : categories.map((cat) => {
              const isActive =
                activeView === "menu" && activeCategory === cat.id;
              const Icon = cat.icon;
              return (
                <button
                  key={cat.id}
                  ...
                  className={`flex flex-col items-center justify-center gap-1.5 w-[52px] h-[58px] rounded-xl mx-auto transition-colors relative ${
                    isActive
                ...
                >
                  ...
                  />
                  <span
                    className={`text-[10px] font-extrabold tracking-wide text-center leading-tight line-clamp-1 ${
      ...
      <button
        ...
        className={`flex flex-col items-center justify-center gap-1.5 w-[52px] h-[58px] rounded-xl mx-auto transition-colors relative mt-auto mb-2 ${
          activeView === "online"
            ? "text-[#357252] bg-[#edf6f1]"
            : "text-gray-400 hover:bg-gray-50 hover:text-gray-600"
        }`}
