  ...
  return (
    ...
    >
      <DialogHeader className="px-6 pt-5 border-b border-gray-100 flex flex-row items-center justify-between text-left">
        <DialogTitle className="text-[20px] font-black tracking-tight text-[#1B3A57]">
          {t("title")}
        </DialogTitle>
      </DialogHeader>
      ...
      <div className="flex flex-col p-6 pt-4 gap-0">
        {shortcuts.map((shortcut, idx) => (
          ...
          >
            <span className="text-[15px] font-medium text-[#1B3A57]">
              {shortcut.action}
            </span>
            <div className="bg-[#f4f6f8] text-[#1B3A57] text-[13px] font-bold px-3 py-1.5 rounded-[6px] min-w-[36px] flex items-center justify-center border border-gray-200 shadow-sm">
              {shortcut.key}
            </div>
          </div>
        ))}
