      ...
      {categories.map((cat, idx) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center justify-center gap-1 w-full h-[60px] transition-colors relative ${
              isActive
          ...
          >
            ...
            <Icon
              className={`w-[20px] h-[20px] ${isActive ? "text-[#357252]" : "text-gray-400"}`}
              strokeWidth={isActive ? 2.5 : 2}
            />
            <span
              className={`text-[9px] font-bold tracking-wide ${isActive ? "text-[#357252]" : "text-gray-400"}`}
            >
              {cat.label}
            </span>
          </button>
