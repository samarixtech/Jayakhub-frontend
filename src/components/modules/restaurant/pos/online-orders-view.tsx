...
import { Bell, ChefHat, CheckCircle2, Check, X, Phone, Bike, User } from "lucide-react";
import Image from "next/image";

type OrderStatus = "incoming" | "preparing" | "ready";
...
export default function OnlineOrdersView() {
    ...
    const [orders, setOrders] = useState<Order[]>([
        {
            ...
            customerName: "Sara M.",
            timeAgo: "0m ago",
            items: ["2x Truffle Mushroom Burger", "1x French Fries"],
            total: 38.85,
            status: "incoming",
        },
        {
            ...
            customerName: "Ali K.",
            timeAgo: "1m ago",
            items: ["1x Truffle Mushroom Burger"],
            total: 16.8,
            status: "incoming",
        },
        {
            id: "JH-1141",
            customerName: "Layla Z.",
            timeAgo: "2m ago",
            items: ["1x Crispy Chicken Deluxe", "1x Tiramisu"],
            total: 21.53,
            status: "incoming",
        },
    ]);
    ...
    const handleMarkReady = (orderId: string) => {
        setOrders((prev) =>
            prev.map((o) => (o.id === orderId ? { ...o, status: "ready", handoffStage: "rider_assigned" } : o))
        );
    };
        ...
        setOrders((prev) =>
            prev.map((o) => {
                if (o.id === orderId && o.status === "ready") {
                    return { ...o, handoffStage: o.handoffStage === "rider_assigned" ? "handoff_code" : "rider_assigned" };
                }
                return o;
            })
        );
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex-1 flex items-center justify-center gap-2 py-3 sm:py-4 px-4 rounded-xl border-2 transition-colors ${isActive
                            ...
                            >
                                ...
                                <Icon
                                    className={`w-[16px] h-[16px] sm:w-[18px] sm:h-[18px] ${isActive ? "text-[#357252]" : "text-gray-400"
                                        }`}
                                    strokeWidth={2.5}
                                />
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 pt-0 space-y-4">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-6 justify-between md:justify-end shrink-0 w-full md:w-auto mt-2 md:mt-0 pt-3 md:pt-0 border-t border-gray-100 md:border-t-0">
                            {order.status === "preparing" && (
                                <div className="flex items-center gap-1.5 px-3 py-1 bg-emerald-50 rounded-full border border-emerald-100 hidden md:flex">
                            ...
                            {order.status === "ready" && (
                                <div className="flex items-center gap-1.5 px-3 py-1 bg-[#fff8eb] rounded-full border border-[#ffe4b5] hidden md:flex shrink-0">
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="font-black text-[#1f2937] text-[15px]">Zain M.</span>
                                                    <div className="flex items-center gap-1.5 text-gray-400 mt-0.5">
                                                        <Phone className="w-3 h-3" />
                                                        <span className="text-[12px] font-medium">+964 750 987 6543</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="relative px-2 mb-6">
                                            ...
                                            <div className="flex justify-between relative z-10 text-center">
                                                <div className="flex flex-col items-center gap-2 w-10">
                                                    ...
                                                    <span className="text-[9px] font-black text-[#357252]">Assigned</span>
                                                </div>
                                                <div className="flex flex-col items-center gap-2 w-10">
                                                    ...
                                                    <span className="text-[9px] font-black text-[#357252]">En Route</span>
                                                </div>
                                                <div className="flex flex-col items-center gap-2 w-10">
                                                    <div className="w-5 h-5 rounded-full bg-gray-200 ring-4 ring-white"></div>
                                                    <span className="text-[9px] font-bold text-gray-400">Arriving</span>
                                                </div>
                                                <div className="flex flex-col items-center gap-2 w-10">
                                                    <div className="w-5 h-5 rounded-full bg-gray-200 ring-4 ring-white"></div>
                                                    <span className="text-[9px] font-bold text-gray-400">Arrived</span>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="text-center">
                                            <span className="font-bold text-gray-500 text-[13px]">ETA: </span>
                                            <span className="font-black text-[#357252] text-[13px]">~10 min</span>
                                        </div>
                                    </>
                                ) : (
                                    <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-300">
                                        <div className="flex items-center justify-between mb-4 cursor-pointer" onClick={() => handleHandoffToggle(order.id)}>
                                            <div className="flex items-center gap-3">
                                                <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-100 border-2 border-white shadow-sm shrink-0">
                                                    <Image
                                                        src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&h=150&fit=crop&crop=faces"
                                        ...
                                        <div className="relative px-2 mb-6">
                                            ...
                                            <div className="flex justify-between relative z-10 text-center">
                                                ...
                                                <div className="flex flex-col items-center gap-2 w-10">
                                                    ...
                                                    <span className="text-[9px] font-black text-[#357252]">Arriving</span>
                                                </div>
                                                <div className="flex flex-col items-center gap-2 w-10">
                                                    <div className="w-5 h-5 rounded-full bg-[#357252] flex items-center justify-center ring-4 ring-white border-[3px] border-white">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-white"></div>
                                                    </div>
                                                    <span className="text-[9px] font-black text-[#357252]">Arrived</span>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="text-center mb-4">
                                            <span className="font-bold text-[#6b7280] text-[14px]">ETA: </span>
                                            <span className="font-black text-[#205139] text-[14px]">Now</span>
                                        </div>
                                        ...
                                        <div className="flex-1 bg-[#f0faef] border border-dashed border-[#a7f3d0] rounded-xl p-5 flex flex-col items-center justify-center gap-2 text-center relative overflow-hidden group">
                                            <span className="text-[12px] font-black tracking-widest text-[#9ca3af] uppercase relative z-10 mb-1">Order ID for Rider</span>
                                            <span className="font-black text-[40px] text-[#357252] tracking-wider relative z-10 leading-none mb-1">{order.id}</span>
                                            <p className="text-[13px] font-bold text-[#6b7280] max-w-[280px] leading-relaxed relative z-10">Give this Order ID to the rider for confirmation</p>
                                            ...
                                            >
                                                <div className="w-5 h-5 bg-[#0ea5e9]/0 text-[#00ff00] bg-white flex items-center justify-center rounded-[3px]">
                ...
                {currentOrders.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-48 sm:h-64 text-gray-400">
                        {activeTab === "incoming" && <Bell className="w-12 h-12 mb-4 text-gray-200" />}
                        {activeTab === "preparing" && <ChefHat className="w-12 h-12 mb-4 text-gray-200" />}
                        {activeTab === "ready" && <CheckCircle2 className="w-12 h-12 mb-4 text-gray-200" />}
