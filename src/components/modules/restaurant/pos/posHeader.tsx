...
export default function POSNavbar() {
    const { cartItems, setIsCartOpen, setIsSidebarOpen, isSidebarOpen } = usePOS();
        ...
        <nav className="h-[64px] shrink-0 bg-[#357252] text-white flex items-center justify-between px-3 sm:px-6 z-20 relative">
            ...
            <div className="flex items-center gap-3 sm:gap-5">
                ...
                <div className="hidden lg:flex items-center gap-4">
                    <button onClick={() => setIsPendingOrdersOpen(true)} className="relative p-1 hover:bg-white/10 rounded-full transition-colors cursor-pointer text-left">
                        <Clock className="w-[20px] h-[20px] text-white" />
                        <div className="absolute -top-0 -right-0.5 w-[14px] h-[14px] bg-[#ef4444] rounded-full flex items-center justify-center text-[9px] font-bold text-white border-2 border-[#357252]">1</div>
                    </button>

                    <button onClick={() => setIsKeyboardOpen(true)} className="p-1 hover:bg-white/10 rounded-full transition-colors relative group">
                    ...
                    <Link href="/restaurant/dashboard" className="p-1 hover:bg-white/10 rounded-full transition-colors text-white ml-2 block">
                        <LogOut className="w-[20px] h-[20px]" />
                    </Link>
                </div>
            </div>

            <KeyboardShortcutsModal open={isKeyboardOpen} onOpenChange={setIsKeyboardOpen} />
            <PendingOrdersSidebar open={isPendingOrdersOpen} onOpenChange={setIsPendingOrdersOpen} />
            <CloseRegisterModal open={isCloseRegisterOpen} onOpenChange={setIsCloseRegisterOpen} />
        </nav>
    );
}
