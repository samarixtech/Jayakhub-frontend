...
import { UtensilsCrossed, ShoppingCart, MapPin, Bike } from "lucide-react";
...
export default function HowItWorks() {
  ...
  return (
    <section
      ref={sectionRef}
      className="bg-[#F8FAFC] relative overflow-hidden min-h-screen flex flex-col justify-center"
    >
          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-16 left-[60%] w-[80%] h-0.5 bg-gradient-to-r from-[#E2E8F0] to-transparent" />
              )}
              ...
              >
                {/* Step Number */}
                <div
                  className={`absolute -top-6 left-8 w-12 h-12 bg-gradient-to-br ${step.color} rounded-2xl flex items-center justify-center text-white font-bold text-lg shadow-lg transform transition-transform group-hover:scale-110 group-hover:rotate-6`}
                >
                  {step.number}
                </div>
                ...
                {/* Icon */}
                <div
                  className={`w-14 h-14 ${step.bgColor} rounded-2xl flex items-center justify-center mb-6 mt-4 transition-transform group-hover:scale-110`}
                >
                  <step.icon
                    className="w-8 h-8"
