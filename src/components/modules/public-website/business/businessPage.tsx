...
export default function BusinessPage() {
  ...
  return (
    <div className="bg-white">
      ...
      >
        <div
          className={`max-w-4xl mx-auto text-center transition-all duration-700 ${visibleSections.intro ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
        >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {featureKeys.map((key, index) => {
              ...
              return (
                <div
                  key={key}
                  className={`bg-white rounded-2xl p-6 border border-gray-100 hover:border-primary/30 hover:shadow-lg transition-all duration-500 ${visibleSections.features ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                    }`}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {stepKeys.map((key, index) => (
              <div
                key={key}
                className={`text-center transition-all duration-700 ${visibleSections.how ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                  }`}
          ...
          <div className="grid md:grid-cols-3 gap-8 items-stretch">
            {planKeys.map((plan, index) => {
              ...
              return (
                <div
                  key={plan}
                  className={`rounded-3xl transition-all duration-700 flex flex-col ${highlighted
                    ? "bg-primary text-white shadow-2xl p-10 -my-4"
                ...
                >
                  <div className="mb-6">
                    <span
                      className={`text-sm font-semibold px-3 py-1 rounded-full ${highlighted ? "bg-white/10 text-white/90" : "bg-primary/10 text-primary"
                        }`}
                    >
                  ...
                  <Button
                    asChild
                    className={`w-full mt-auto ${highlighted
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonialKeys.map((key, index) => (
              <div
                key={key}
                className={`bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl p-6 transition-all duration-700 ${visibleSections.testimonials ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                  }`}
                style={{ transitionDelay: `${index * 100}ms` }}
              >
