...
import { PopularRestaurantsSectionProps } from "@/components/modules/discovery/discovery.types";
...
> = ({ isPending, restaurants, isLoggedIn, onAction }) => {
  ...
  const tPage = useTranslations("Discovery.restaurantsPage");
  return (
    <section className="mb-4">
      <SectionHeader
        title={tPage("nearbyCount", { count: (restaurants || []).length })}
        onAction={onAction}
      />
      {isPending ? (
