...
export const RestaurantHero: React.FC<RestaurantHeroProps> = ({
  ...
  const t = useTranslations("Discovery.restaurantHero");
  const { currency } = useCLC();
  const isClosed = restaurant?.isOpen === false;
  return (
    <div className="w-full relative md:rounded-2xl overflow-hidden">
      <div className={`h-[200px] md:h-[350px] w-full relative overflow-hidden ${isClosed ? "grayscale" : ""}`}>
      ...
      <div className="max-w-7xl mx-auto relative z-20 -mt-12 md:-mt-24 mb-6 md:mb-8">
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-lg p-4 md:p-6 lg:p-8 flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6">
          <div className={`w-24 h-24 md:w-32 md:h-32 rounded-xl overflow-hidden shadow-md border-4 border-white shrink-0 -mt-14 md:mt-0 bg-white relative z-30 ${isClosed ? "grayscale" : ""}`}>
            <Image
              width={250}
              height={250}
              src={profileUrl}
          ...
          <div className={`flex-1 text-center md:text-left w-full mt-1 md:mt-0 ${isClosed ? "opacity-60" : ""}`}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-2 md:mb-3">
              <div className="flex flex-col md:flex-row md:items-center gap-2 w-full md:w-auto">
                ...
                {isClosed ? (
                  <Badge className="bg-red-50 text-red-600 hover:bg-red-100 border-none w-fit mx-auto md:mx-0 text-[10px] md:text-xs">
                    {t("closed")}
                  </Badge>
                ) : (
                  deliveryFee === 0 && (
